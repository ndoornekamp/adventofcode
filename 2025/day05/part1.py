from textwrap import dedent


def solve(input: str) -> int:
    fresh_ranges_str, available = input.strip().split("\n\n")

    fresh_ranges: list[tuple[int, int]] = []
    for fresh_range_str in fresh_ranges_str.split("\n"):
        range_start, range_end = fresh_range_str.split("-")
        fresh_ranges.append((int(range_start), int(range_end)))

    ingredients = [int(ingredient) for ingredient in available.strip().split("\n")]

    ans = 0
    for ingredient in ingredients:
        if any(s <= ingredient <= e for s, e in fresh_ranges):
            ans += 1

    return ans


def test_solve():
    input = dedent("""
        3-5
        10-14
        16-20
        12-18

        1
        5
        8
        11
        17
        32
    """)
    assert solve(input) == 3


if __name__ == '__main__':
    with open('input.txt') as f:
        input = f.read()

    ans = solve(input)
    print(ans)
